...
public final class PlayerPainter {
    ...
    /**
     * regarding to the PlayerID, LifeState, Direction and his position, returns
     * the right image of player.
     * 
     * @param tick
     *            current tick
     * @param player
    ...
    public static byte byteForPlayer(int tick, Player player) {
        ...
        if (state == State.DYING) {
            ...
            int comp = player.direction().isHorizontal() ? player.position().x()
                    : player.position().y();
            if (comp % 4 == 1)
                output += 1;
            else if (comp % 4 == 3)
