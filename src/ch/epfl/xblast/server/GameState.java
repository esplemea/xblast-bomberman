...
public final class GameState {
	...
	public GameState(int ticks, Board board, List<Player> players, List<Bomb> bombs, List<Sq<Sq<Cell>>> explosions,
		...
		this.bombs = Objects.requireNonNull(Collections.unmodifiableList(new ArrayList<>(bombs)));
		this.explosions = Objects.requireNonNull(Collections.unmodifiableList(new ArrayList<>(explosions)));
		this.blasts = Objects.requireNonNull(Collections.unmodifiableList(new ArrayList<>(blasts)));
	}
	...
	public GameState next(Map<PlayerID, Optional<Direction>> speedChangeEvents, Set<PlayerID> bombDropEvents) {
		...
		for (Bomb bomb : bombs) {
			if (bomb.fuseLengths().tail().isEmpty() || blastedCells.contains(bomb.position())) {
		...
		bombsOutput.addAll(newlyDroppedBombs(orderPlayer, bombDropEvents, bombs));

		Set<Cell> consumedBonuses = new HashSet<>();
		Map<PlayerID, Bonus> playerBonuses = new HashMap<>();
		for (Player p : alivePlayers()) {
			Cell playerActualCell = p.position().containingCell();
			if (board.blockAt(playerActualCell) == Block.BONUS_BOMB && p.position().isCentral()) {
				consumedBonuses.add(playerActualCell);
				playerBonuses.put(p.id(), Bonus.INC_BOMB);
			} else if (board.blockAt(playerActualCell) == Block.BONUS_RANGE && p.position().isCentral()) {
				consumedBonuses.add(playerActualCell);
		...
		return new GameState(ticks + 1, boardOutput, playersOutput, bombsOutput, explosionsOutput, blasts1);
	}
	...
	private static Board nextBoard(Board board0, Set<Cell> consumedBonuses, Set<Cell> blastedCells1) {
		...
		for (Cell c : Cell.ROW_MAJOR_ORDER) {
			...
			if (b == Block.DESTRUCTIBLE_WALL && blastedCells1.contains(c)) {
				...
				blocks.add(board0.blocksAt(c).tail().limit(Ticks.BONUS_DISAPPEARING_TICKS)
	...
	private static List<Player> nextPlayers(List<Player> players0, Map<PlayerID, Bonus> playerBonuses,
		...
		for (Player player : players0) {
			...
			if (speedChangeEvents.containsKey(player.id())) {
				askedDir = speedChangeEvents.get(player.id()).orElse(player.direction());
			...
			if (!((position.distanceToCentral() == BOMB_RADIUS + 1)
				...
				if (position.isCentral()) {
					if (!board1.blockAt(position.containingCell().neighbor(askedDir)).castsShadow()) {
	...
	private static List<Bomb> newlyDroppedBombs(List<Player> players0, Set<PlayerID> bombDropEvents,
		...
		for (Player player : players0) {
			...
			if (bombDropEvents.contains(player.id()) && player.isAlive() && player.maxBombs() > totalBombs && state) {
