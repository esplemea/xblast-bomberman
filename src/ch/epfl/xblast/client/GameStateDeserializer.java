...
public final class GameStateDeserializer {
    ...
    private static ImageCollection EXPLOSION_IMAGES = new ImageCollection(
            "explosion");
    private static ImageCollection SCORE_IMAGES = new ImageCollection("score");
    private static ImageCollection PLAYER_IMAGES = new ImageCollection(
            "player");
    ...
    public static GameState deserializeGameState(List<Byte> bytes) {
        ...
        return new GameState(desearializePlayer(playerBytes),
                ...
                deserializeScore(playerBytes), deserializeTime(bytes.get(end)));
    }

    /**
     * @param bytes
     *            Block's bytes
     * @return The deserialized Blocks
     */
    private static List<Image> deserializeBlocks(List<Byte> bytes) {
        List<Image> output = new ArrayList<>(
                Collections.nCopies(Cell.COUNT, null));
     ...
     * @param bytes
     *            Players' bytes
     * @return The List of images for the score area
     */
     ...
     * @param ordinal
     *            Player ID's ordinal
     * @return The list of image representing the score area for one single
     *         player
     */
    ...
    private static List<Player> desearializePlayer(List<Byte> bytes) {
        ...
        for (int i = 0; i < bytes.size() / 4; ++i) {
            output.add(new Player(PlayerID.values()[i], bytes.get(4 * i),
                    new SubCell(Byte.toUnsignedInt(bytes.get(4 * i + 1)),
                            Byte.toUnsignedInt(bytes.get(4 * i + 2))),
